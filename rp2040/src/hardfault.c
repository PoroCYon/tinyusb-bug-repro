...
#define AAA __attribute__((__no_sanitize_address__, __force_inline__))
#define UGH __attribute__((__no_sanitize_address__, __used__, __noinline__))

AAA static inline void __not_in_flash_func(print_u32)(uint32_t v) {
...
AAA static inline void __not_in_flash_func(prints)(const char* s) {
	for (; *s; ++s) {
		while (!uart_is_writable(uart_default));
...
UGH void __not_in_flash_func(isr_hf_stuff)(uintptr_t orig_sp, uint32_t* stackframe) {
